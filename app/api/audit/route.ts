...
export async function POST(req: NextRequest) {
  ...
  const row = {
    ...
    answers,
    scores: waste,
    overall_score: waste.totalWaste,
    score_band: waste.totalWaste > 500000 ? 'Critical' : waste.totalWaste > 200000 ? 'High' : waste.totalWaste > 50000 ? 'Moderate' : 'Low',
    total_waste: waste.totalWaste,
    findings: {
      businessType: answers.business_type ?? null,
      followUpProcess: answers.follow_up_process ?? null,
      crmStatus: answers.crm_status ?? null,
      outOfHours: answers.out_of_hours ?? null,
      remindersStatus: answers.reminders_status ?? null,
